...
type QueueItem = {
  ...
  tableId: string;
  teamName: string;
  status: "In Progress" | "Next" | "Prepare" | "Queued" | "Finished" | "Absent" | "Delayed" | "Completed";
...
export default function JudgeAdminFlowPage() {
  ...
  function moveQueueForward(action: QueueItem["status"]) {
    setQueue((current) =>
      current.map((item) => {
        if (item.tableId !== selectedTable) return item;
        if (item.status === "In Progress") return { ...item, status: action };
        if (["Finished", "Absent", "Delayed", "Completed"].includes(action)) {
  ...
  function handleFinish() {
    ...
    if (!(hasSignature && signatureForTeamId === currentTeam.id)) {
    ...
    <main style={{ minHeight: "100vh", background: "#f8fafc", color: "#0f172a", fontFamily: "Arial, Helvetica, sans-serif" }}>
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: 24, display: "grid", gap: 20 }}>
        <section style={heroStyle}>
          <span style={badgeStyle}>Judge → Admin Flow</span>
          <h1 style={{ marginTop: 16, marginBottom: 12 }}>Judge result handoff to admin</h1>
          <p style={mutedStyle}>
            When the current team signs and the judge presses <strong>Finish</strong>, the <strong>score</strong>, <strong>time</strong>,
            <strong> notes</strong>, and <strong>signature captured status</strong> are sent to the admin results panel.
          </p>
        </section>
        ...
        <div style={{ display: "grid", gap: 16, gridTemplateColumns: "1.1fr 0.9fr" }}>
          <section style={cardStyle}>
            ...
            <div style={{ ...rowStyle, marginTop: 16 }}>
              <button style={secondaryButtonStyle} onClick={() => moveQueueForward("Completed")}>Next</button>
              <button style={warningButtonStyle} onClick={() => moveQueueForward("Delayed")}>Delay</button>
              <button style={dangerButtonStyle} onClick={() => moveQueueForward("Absent")}>Absent</button>
              <button style={successButtonStyle} onClick={handleFinish}>Finish & Send to Admin</button>
            </div>
          ...
          <section style={cardStyle}>
            ...
            <p style={mutedStyle}>This is what admin receives after the judge clicks Finish.</p>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {adminResults.length ? adminResults.map((item) => (
                    <tr key={item.id}>
                      <td style={tdStyle}><strong>{item.teamName}</strong><div style={smallMutedStyle}>{item.status}</div></td>
                      <td style={tdStyle}>{item.tableLabel}</td>
                      <td style={tdStyle}>{item.score}</td>
                      <td style={tdStyle}>{item.time} sec</td>
                      <td style={tdStyle}>{item.notes}</td>
                      <td style={tdStyle}><span style={pillStyle}>{item.signatureCaptured ? "Captured" : "Missing"}</span></td>
                    </tr>
                  )) : (
                    <tr>
                      <td colSpan={6} style={{ ...tdStyle, color: "#475569" }}>No finished results received yet.</td>
                    </tr>
                  )}
                </tbody>
              </table>
